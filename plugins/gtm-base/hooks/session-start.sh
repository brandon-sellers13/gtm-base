...
if command -v timeout > /dev/null 2>&1; then
  output=$(timeout 14 python3 "$plugin_root/scripts/session_start.py" --client "$client" 2> /dev/null)
else
  output=$(python3 "$plugin_root/scripts/session_start.py" --client "$client" 2> /dev/null)
fi
status=$?
