...
- A no needs their own words, so ask this skill for somewhere to put them
  first:

  ```
  python3 scripts/confirm.py --new-words-file reason
  ```

  It prints `words=<path>`. Write what they said into that path with your
  file-writing tool, and then record the answer:

  ```
  python3 scripts/confirm.py --question <id> --answer no --reason-file <the path it printed>
  ```

  The file is read once and then taken away, and a path this skill did not
  print is refused, so their words never go in the command itself and no other
  file on the computer can be read this way.
