...
- Still open after the final pass, said plainly. A words file is a file, so a command could fill one with the contents of any document on the computer; what stands in front of that is the preview, which shows the whole of what would be written before anybody says yes, and the same is true of a prepared change, whose identifier is a field in a file anything may write. Client settings in a project folder and shell profiles are not watched, and either can change how the assistant behaves. An invisible character, and a change to who may read or write a file, do not show up in the difference a hand edit is approved against. A note left by an older build is ignored rather than finished. Because this release answers "never reviewed" to every base, a folder wrongly taken for a base is refused every send until the first backup review ships. Whether asking rather than refusing really puts a prompt in front of somebody under the client's own modes for skipping prompts has not been tested. And Astra's third look did not run, for want of usage, and is outstanding.
