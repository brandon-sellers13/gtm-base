...
- R22. Two triggers, two questions. When a file passes the confirmation threshold, the owner is shown the file and asked in plain words whether it is still true as written. When a newer ledger entry affects the file, the owner is shown the entry beside the file and asked whether the file already reflects that decision; a yes is recorded against that entry id, and a no opens the proposal with the entry as evidence. The threshold is one number of days per base (starting default 30) with no per-file setting in v1. (Amended 2026-09-19 by Amendment r2.5 of `docs/plans/2026-09-04-001-feat-current-without-integrations-plan.md`. Both questions still exist and both still work the same way. What changed is where they are asked. They are no longer raised on their own at the start of a session. They are asked inside a review the person asks for by saying "review my base", and the second one is also asked at the moment a document that a recorded context change has overtaken is about to be used. The word the person reads for what the base tracks is now "context change", not "decision". The map is never asked about at all, and it is left out by its kind rather than given a confirmation line, because a confirmation line would only bring it back after the threshold.)
- R23. The questions are raised by the instruction the session-start hook injects for the AI to act on at the first turn, on startup and resume only, never when the client compresses a long session. At most one file is asked per session start; the rest are listed for `stale-check`, and a "not now" suppresses that file's question for a set number of days. There is no separate confirmation skill. (Amended 2026-09-19 by Amendment r2.5 of `docs/plans/2026-09-04-001-feat-current-without-integrations-plan.md`. The base is quiet by default. At the start of a session it loads its context and says nothing: no question leads the first reply and no question identifier is issued. The question identifiers, the record of what was asked, "not now", and a no that becomes a prepared change all still work exactly as written here, but they now live inside the review the person asks for and inside the moment-of-use flag. The rest of this requirement stands.)
