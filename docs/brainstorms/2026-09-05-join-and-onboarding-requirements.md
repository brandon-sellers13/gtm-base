...
- J8. Join drafts two required files, in this order: the ICP and the positioning. The map's two settings are written with defaults and not drafted. Voice and every other file are offered as empty and named. Each draft carries an owner email. (Amended 2026-09-19, r2.4 section A. J8 used to require a third drafted file, a ledger entry holding one recent decision. That step was not understood by the people it was written for, and on the day it was written it changed nothing they could see, so it is no longer a required drafted file. What the base tracks is now called a context change, and one is offered at the closing as an optional question rather than drafted as a document. See Amendment r2.4 section B and Amendment r2.5 of `docs/plans/2026-09-04-001-feat-current-without-integrations-plan.md`.)
- J9. Every draft is shown before it is written, with approve, edit, or skip, and each draft ends with "what is wrong with this?" so approval is not the only signal. Nothing is written without a yes. The base folder is created on the first approved file, and approving a file records a confirmation for it dated that day with the trigger "drafted," so the base is not flagged stale against itself. (Amended 2026-09-19, r2.4 sections A and B. "Every draft" now means the two required files. A context change captured at the closing is a separate thing: it is shown whole before it is written, every field in it is the person's to correct, and it can never confirm the documents it affects, because the person is asked about each affected document separately.)
- J10. Where the sources do not cover a required file, join asks the fewest plain questions needed and drafts from the answers. (Amended 2026-09-19, r2.4 section A: a required file means one of the two.)
...
- J17. At the end of user one's session join runs `stale-check` on the new base and shows its first finding, so the person leaves with one thing the base did, then states in plain words that the base lives in this folder on this machine only and how to come back to it. (Amended 2026-09-19, r2.4 section A. On a base with two confirmed files and nothing recorded about what has changed, the finding is an honest baseline: both documents were confirmed today, no context change is recorded, the base therefore cannot yet check whether a change has made either document out of date, and here is the date each confirmation comes up for review. It never says that nothing is out of date and it never says there is no date to watch. The full order is in `plugins/gtm-base/skills/join/references/closing-rules.md`.)
...
## Success Criteria
- SC1. A person who has never spoken to Brandon installs the plugin, opens Claude Code, and has a first approved file within ten minutes of saying "set up," and an approved ICP and positioning, each with an owner email, by the end of the session, on the paste path with no source connected, without typing a command to start. (Amended 2026-09-19, r2.4 section A: the ledger entry is no longer one of the files this criterion counts. The first feature's own SC1, a hand-entered context change that yields a correct flag and a drafted proposal, is a different criterion in a different document and is untouched; it keeps its own test and that test is rerun after every unit of this phase.)
- SC2. The same run with at least one source connected is timed separately and reported; join quotes no duration until then.
- SC3. At the end of session one, `stale-check` produces one finding on the new base. (Amended 2026-09-19, r2.4 section A: on a base with nothing recorded yet, the honest baseline finding satisfies this criterion. It is a truthful result, and it is not evidence that the base caught a change, so it is never reported as one.)
