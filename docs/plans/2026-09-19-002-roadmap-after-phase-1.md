...
| Fable M8 | 3.2, 3.3 | Both read `context/work/`, which exists in neither the template nor the real base. Closed by Brandon on 2026-09-19: work in flight stays outside the base as a source, so the in-flight contradiction feature is cut from both skills. |
