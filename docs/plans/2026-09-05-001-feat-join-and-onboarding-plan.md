...
- J17 (closing finding and message): Unit 5. (Amended 2026-09-19, r2.4 section A: on a base with nothing recorded yet, the finding is the honest baseline. Built in Unit 1.2 of `docs/plans/2026-09-19-001-feat-base-that-produces-work-plan.md`.)
...
- SC1 (first approved file within ten minutes on the paste path; two files by session end): Units 3, 4, 5, measured in Unit 8's walkthrough. (Amended 2026-09-19, r2.4 section A: the count was three and is now two, because the ledger entry is no longer a drafted step.)
- SC2 (connected-source run timed separately): Unit 8.
- SC3 (one finding at session end): Unit 5. (Amended 2026-09-19, r2.4 section A: the honest baseline finding satisfies this, and is never reported as evidence that the base caught a change.)
...
- No brief pipeline. The two drafts are produced directly from the fenced sources with the donor's section and never-hallucinate rules. The ProfileBase conductor-and-specialist stage is deferred, with the trigger being a walkthrough where direct drafting fails on source volume. (Amended 2026-09-19, r2.4 sections A and G. The count was three. The deferral is also no longer open ended: it is now gated on a fidelity replay of three cases, recorded in `docs/walkthroughs/2026-09-fidelity-replay.md`, and a fail builds the summarize-first stage in the same release.)
...
| A base is joined at the first approved file; a required file is complete when it exists with a `status` other than `skipped`; the daily block checks the two required files and, when one is missing or skipped, offers to continue setup instead of asking a confirmation question, issuing no question id and writing nothing to the asked log. (Amended 2026-09-19, r2.4 section A: the count was three. Amended again by r2.4 section F: a file that carries an unanswered `[your call: ...]` marker is not complete either, and that state is defined independently of `status`.) | The folder is the resume state; joining early makes the parent-folder rule and the hook work immediately; a skip must leave a way back; a half-made base must not pollute the yes-rate denominator. | Join at session end; presence-only completeness. |
...
| The closing finding is computed by one stale-check first-run call after the last approve or date correction, never cached, in the order set out in `plugins/gtm-base/skills/join/references/closing-rules.md`: a required file the person skipped; a file carrying a question nobody has answered; a file whose source material predates a context change that affects it and is still open; the honest baseline when nothing has been recorded yet; otherwise the statement that nothing is out of date, naming the review date it will watch. The finding text never claims more than the dates show. (Amended 2026-09-19, r2.4 section A. The order used to have four places and the fourth said nothing was out of date even when there was nothing to check against, which mistook missing evidence for a base known to be current. The honest baseline replaces that. The source-age place is also narrowed to the files a change affects and to changes still open, and it no longer keys on the setup run's identifier, because a context change captured at the closing carries none.) | Origin J17 promises one real thing; the rule makes it deterministic and honest. | Hoping for a finding. |
...
*(Amended 2026-09-19, r2.4 section A. This unit shipped as written and the text below is the record of what was built. Setup now drafts two documents, the ideal customer profile and the positioning. The middle prompt, `draft-ledger-entry.md`, is no longer part of setup's drafting, and the sentences in this unit that say "three" describe what shipped rather than what setup does from now on. The prompt file itself and the entry shape it produces stay in the plugin, because the same shape is used for a context change captured at the closing. The change is built in Units 1.1 and 1.2 of `docs/plans/2026-09-19-001-feat-base-that-produces-work-plan.md`.)*
...
*(Amended 2026-09-19, r2.4 sections A and B. This unit shipped as written and the text below is the record of what was built. Two things change from now on. Setup runs two drafts rather than three, so the scenario that expects three files and three drafted lines becomes two files and two drafted lines. The closing finding follows the amended order in `plugins/gtm-base/skills/join/references/closing-rules.md`, which puts an honest baseline where the old order said nothing was out of date on a base that had nothing to check against. The closing also gains one optional question about whether anything in the context of the business changed, which is not a drafted document, never confirms the documents it affects, and can be answered with "skip". The change is built in Units 1.1, 1.2 and 1.5 of `docs/plans/2026-09-19-001-feat-base-that-produces-work-plan.md`.)*
...
2. When the record has gone quiet for the window, the daily block asks once whether anything was decided, with a dismissal that expires, at most once a session, outside the yes-rate accounting.
   ...
   *(Amended 2026-09-19 by Amendment r2.5 of `docs/plans/2026-09-04-001-feat-current-without-integrations-plan.md`, and recorded as requirement P17 of `docs/plans/2026-09-19-001-feat-base-that-produces-work-plan.md`. The base is quiet at the start of a session, so this ask does not live in the daily block. It is asked inside the review a person asks for by saying "review my base", still at most once a session, still outside the yes-rate accounting. The dismissal lasts the base's confirmation threshold, which is 30 days unless the map sets another number, and then the ask returns. A record with nothing in it is behind immediately rather than after the window, which is what the shipped code already does.)*
...
*(Amended 2026-09-19, revision 2 of `docs/plans/2026-09-19-001-feat-base-that-produces-work-plan.md`, requirement P21, and awaiting Brandon's yes to call 5 of that plan's "For Brandon" table. "Sees each cleaned body whole" and section H's "never paragraphs of evidence" contradicted each other once there were twelve documents to approve. The resolution separates the interaction from the artifact. In the conversation the person gets a short wrapper for each document: what was removed, the opening lines, and where the whole cleaned file is. The whole cleaned file is written out and is there to be read in full before any answer is given, and the approval is bound to a hash of exactly those bytes. Nothing about the strength of the approval changes; what changes is that the full text is a document the person opens rather than a wall of text in the conversation. Every other clause in this section stands as written.)*
...
*(Amended 2026-09-19, revision 2 of `docs/plans/2026-09-19-001-feat-base-that-produces-work-plan.md`, requirement P28. Two changes. The four short lines describe a context change, not a decision, and they read: what changed, why, what it affects, when to look again. And rule 2 above, that what is shown can be read in a few seconds and is never paragraphs of evidence, applies to the interaction and not to a complete document the person is being asked to approve. A whole document shown for approval is its own thing, separate from the short wrapper around it, and the rule does not cut it down. That is how this section and section D both hold at once.)*
